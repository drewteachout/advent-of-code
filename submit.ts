...
import { formatTime, getAppRoot, wait } from "utilities/util";
import path from "path";
import { log, SolutionObject, solutionLogKey } from "utilities/log";
...
function getMsUntilActiveCooldownExpiration(solutionLog: SolutionObject[]) {
	const expirations = solutionLog.map(solution => {
		...
		if (previousSubmissions && previousSubmissions.length > 0) {
			const mostRecentSubmission = _.maxBy(previousSubmissions, s =>
				new Date(s.date).getTime()
			)!;
			if (
				mostRecentSubmission.result === "timeout" &&
				mostRecentSubmission.cooldownFinished != undefined
			) {
				return (
					new Date(mostRecentSubmission.cooldownFinished).getTime() -
					new Date().getTime()
				);
			}
		}
...
async function submit() {
	...
	for (const solution of solutionLog) {
		...
		if (!mostRecent) {
			...
			new Date(mostRecent.dateComputed).getTime() < dateComputed.getTime()
		) {
			mostRecent = solution;
		}
		i++;
	...
	if (result.status === 200) {
		...
		if (responseText.includes("That's not the right answer")) {
			...
			if (waitTimeMatch) {
				cooldownFinished = new Date(
					new Date().getTime() + Number(waitTimeMatch[1]) * 1000
				).toJSON();
			}
			mostRecent.submissions.push({
				date: new Date().toJSON(),
			...
			log(
				chalk.greenBright("Correct! One ") +
					chalk.yellowBright("GOLD") +
					chalk.greenBright(" star for you!")
			);
		} else {
			mostRecent.submissions.push({
				date: new Date().toJSON(),
