...
function getDataPath(day: number, year: number) {
	const dataDir = getDayRoot(day, year);
...
function getSolutionPath(day: number, year: number) {
	...
	const templateExtension = path.extname(
		templatePath.substr(0, templatePath.length - 4)
	);
...
function parseArgs() {
	...
	const yearArg =
		yearIndex >= 0 ? args[yearIndex + 1] : String(latestPuzzle.year);
	...
	const rootPath =
		rootPathIndex >= 0
			? args[rootPathIndex + 1]
			: path.join(getAppRoot(), "years");
	...
	const sessionToken =
		sessionTokenIndex >= 0 ? args[sessionTokenIndex + 1] : undefined;
...
async function run() {
	...
	if (settings.suck) {
		if (settings.wait) {
			const latestPuzzleAsOfTomorrow = getLatestPuzzleDate(
